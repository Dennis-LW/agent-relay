...
const tasks = (planText.match(/^- \[ \] /gm) || []).length;
const verify = (planText.match(/^#{1,6}\s+verify\s*\n+\s*`?([^`\n]+)`?/im) || [])[1]?.trim() || "";
if (!tasks || !verify) die("plan needs '- [ ]' tasks and a '## Verify' section");

// Rough, deliberately pessimistic: ~$0.60 per Sonnet worker session on a small task.
const perSession = /haiku/i.test(model) ? 0.15 : /opus/i.test(model) ? 2.5 : 0.6;
...
function summary(rows) {
  ...
  const lines = [
    `# relay benchmark — ${path.basename(planFile)}, model ${model}, judge ${judgeModel}`,
    "",

...
1. Run `{{VERIFY}}`. Then walk through every task's Accept criteria and check them for real (run the command, call the endpoint, open the file, read the test). Do not trust the tick marks.
...
4. For every FAIL or gap that must be fixed, append a task at the END of the task list in `{{PLAN_PATH}}`:
   `- [ ] A<n>: <one-line fix>` with an indented `- Accept: <how to verify>` line. Do not fix code yourself.
